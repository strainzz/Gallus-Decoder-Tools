...
Use it when those figures have to be exact. A person can run the commands while working a decode. An AI agent can call the same commands instead of estimating the arithmetic. The calculation happens on the machine where the package is installed.
...
`python -m gallus_decoder_tools` is the same program before the installed command is on your path. The only dependency is `python-dateutil`.

A plain decode of `New York` is English Ordinal 111, Full Reduction 39, Reverse Ordinal 78, and Reverse Full Reduction 33. `28` is the 7th triangular number, the 4th hexagonal number, and the 2nd perfect number. January 1, 2020 through September 22, 2026 is 6 years, 8 months, and 21 days.

Every response is JSON and carries the credit:
...
Decode uses four everyday ciphers unless you name others. All 32 ciphers are included. `--all` runs every one. The catalog, with the rule for each code, is [docs/ciphers.md](docs/ciphers.md).
